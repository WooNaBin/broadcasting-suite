...
## 2026-09-16 — SDM 공문 연결·이름규칙 / WL 인쇄 칸 1~7

- **스위트:** `—` (재배포 시 브리지 `sync-ui`로 `/schedule` `/worklog` 반영)
- **대상:** ScheduleDataManager / WorkLog / BroadcastNasBridge(UI sync)
- **유형:** 수정 | 기능

### 내용
- **SDM** 새 일정에서 공문 연결: 제목 없이도 연결 가능(날짜만 필수). 제목 비면 파일명에서 채움. 인덱스 API 실패 시에도 일정에 파일명 연결 유지
- **SDM** 공문 파일명 규칙: `YYYYMMDD[_YYYYMMDD]_제목[_강사PA|EV][_장소][_HHmm]` · 기간·강사·장소·시각 파싱
- **SDM** 공문 보기 상단 **이 공문을 일정으로 만들기** → 파싱값·공문 연결 상태로 새 일정 창
- **WL** 날짜선택 인쇄 칸 1~7개(±) · 마지막 칸만 날짜/특이사항 · 칸 수 localStorage

### 확인
- [x] `BroadcastNasBridge/scripts/sync-ui.ps1`
- [ ] 새 일정(제목 비움) → 공문 연결 → 저장 후 공문 보기
- [ ] `20260901_20261011_제목_이름PA_교육원 대강당_1730` 공문 → 일정으로 만들기 필드 확인
- [ ] WL 인쇄 날짜선택 ±로 1~7칸 · 마지막 칸 특이사항

### Win / Mac
- Win·Mac 동일 브라우저 UI (브리지 `/schedule` `/worklog` 또는 단독). OS별 분기 없음.

---

## 2026-09-16 — SDM 다수 추가 표 필드·행 삭제
...
### 내용
- **SDM:** 일정 추가「다수 추가」에서 `createBulkScheduleRow` / `resetBulkScheduleRows`가「일괄 입력」정의에 덮여 헤더·셀 순서가 어긋나던 문제 수정 (일정 제목·강사·준비·행 삭제 복구)
- **SDM:** 다수 추가 헤더「일정 제목」표기 · 일괄 입력 표도 동일 순서로 정리하고 행 삭제 버튼 추가

### 확인
- [ ] Win: 일정 추가 → 다수 추가 — 헤더/입력 칸 일치 · 행 × 삭제
- [ ] Win: 일괄 입력 대화상자 — 일정 제목·행 삭제
- [ ] Mac: 동일(브라우저)

---

## 2026-09-16 — SDM 이번 달 공문 · 달력 표시 옵션
...
### 내용
- **SDM:** 상단「이번 달 공문」버튼 — 보고 있는 달력 연월의 YYYYMMDD 공문 목록, 선택 시 기존 공문 뷰어
- **SDM:** 설정「기타」— 기간일정 날짜표시 `(M.D~M.D)` · 글자색(지정/제목/강사)
- **SDM:** 달력 이전달·다음달 칸 밝은 회색(토·일·공휴일 무관)
- **SDM:** 설정「색상」— 공휴일 배경 사용·색 지정

### 확인
- [ ] Win: 이번 달 공문 목록·뷰어 · 기간표시 옵션 · outside 회색 · 공휴일 배경
- [ ] Mac: 동일(브라우저 UI) · sync-ui

## 2026-09-16 — SR 강사·부가 분할 · 공휴일 제외 · 탭 내보내기 제외
...
### 내용
- **SR:** 추출 시 강사·부가정보가 `,`로 2개 이상이고 개수가 같으면 `제목(부가)`로 일정 자동 분할(기간 동일)
- **SR:** 「공휴일 제외 추출」 체크 — 공휴일 날짜의 공휴일 제목 일정만 제외
- **SR:** 보기 탭에서 「아래의 일정들을 내보내기에서 제외」 — JSON 내보내기 시 해당 탭 일정 제외

### 확인
- [ ] Win: 교구모임 + 강사2·부가2 → 두 일정으로 추출
- [ ] Win: 공휴일 제외 체크 후 현충일 등 제외·일반 일정 유지
- [ ] Win: 탭 제외 체크 후 내보내기 JSON에 해당 제목 없음
- [ ] Mac: 해당 없음(SR Windows 전용) · 브라우저 UI 동일 시 해당

## 2026-09-16 — SR 단어필터 다중 대상 세팅
...
### 내용
- **SR:** 단어 필터 자동 세팅 — 사전준비·시간·장소·부가정보·설명을 체크박스로 여러 개 선택해 한 번에 추가

### 확인
- [ ] Win: 단어필터에서 사전준비+장소 등 복수 체크 후 추가·추출 적용
- [ ] Mac: 해당 없음(SR Windows 전용) · 브라우저 UI 동일 시 해당

## 2026-09-16 — SDM 색상-폰트 · 달력 체크 필터
...
### 내용
- **SDM:** 설정「색상-폰트」조건 추가/저장·기본 글자색·달력 제목/강사 색 적용 복구
- **SDM:** 달력 위 체크 필터(+기어로 조건 추가, 「기타」포함)로 표시 일정 필터

### 확인
- [ ] 색상-폰트 조건 추가·달력 글자색 반영
- [ ] 달력 필터 체크/해제·기타
- [ ] Win / Mac: 동일(브라우저) · sync-ui

## 2026-09-16 — SDM 달력표 삭제 범위(같은 주 이전·다음달 포함)
...
### 내용
- **SDM:** 「현재 보고 있는 달력표의 모든 스케줄 삭제」가 해당 월뿐 아니라 달력에 보이는 같은 주 이전달·다음달 칸 일정도 삭제

### 확인
- [ ] 달력 앞·뒤 outside 칸 일정도 함께 삭제되는지
- [ ] 월 선택 삭제(자료관리)는 해당 월만 유지
- [ ] Win / Mac: 동일(브라우저)

## 2026-09-16 — SDM 달력 공휴일·연휴 표시
...
### 내용
- **SDM:** 달력 날짜 옆에 공휴일명 표시(설·추석 연휴·대체공휴일 포함)
- **SDM:** 공휴일(연휴) 날짜 숫자는 일요일과 같이 빨간색

### 확인
- [ ] 달력에서 설·추석 연휴·어린이날 등 표시·빨간 숫자
- [ ] 토요일이 공휴일이면 숫자도 빨강
- [ ] Win / Mac: 동일(브라우저 UI)

## 2026-09-16 — SDM 현재 달력 삭제 / SR 나누기 기간·내보내기 파일명

- **스위트:** —
- **대상:** ScheduleDataManager / ScheduleReader
- **유형:** 기능 | 수정

### 내용
- **SDM:** 설정 → 자료관리에「현재 보고 있는 달력표의 모든 스케줄 삭제」(달력 보기에서만 활성)
- **SR:** 일정 나누기 시 원본 기간을 잠가 시간만 바꿔도 날짜 유지 · 동기화 시 빈 날짜 덮어쓰기 방지
- **SR:** 내보내기 기본명 `ScheduleConvertedData_{중심yyyymm}_{저장시각}.json`

### 확인
- [ ] SDM 달력 보기에서만 버튼 활성·해당 월 삭제
- [ ] SDM 목록 보기에서는 비활성
- [ ] SR 나누기 후 시간 변경·기간 유지
- [ ] SR 내보내기 파일명
- [ ] Win: SDM 브리지/단독 · SR 로컬
- [ ] Mac: SDM 동일(브라우저) · SR Windows 전용

## 2026-09-16 — SR 보기 탭 · 일정 나누기 다수 복제
...
### 내용
- **SR:** 추출 결과 옆 기어로 보기 탭 설정(제목 `or`/`and`/`=`) · 전체/기본/추가 탭 · 추출·내보내기 무영향
- **SR:** 일정 나누기 — 원본 부가정보·필드 요약 표시, 복제 행 여러 개, 부가/설명/사전준비까지 설정

### 확인
- [ ] SR 보기 탭 추가·전환·내보내기(숨긴 행 유지)
- [ ] SR 일정 나누기 다수 복제·부가 필드
- [ ] Win: ScheduleReader 로컬 UI
- [ ] Mac: 해당 없음(SR Windows 전용) · 브라우저 UI 동일 시 해당

## 2026-09-16 — SDM 연결 삭제·`=` 필터 / SR 축약·단어필터·나누기
...
### 내용
- **SDM:** 같은 제목·강사 일정이 여러 날짜에 있으면 삭제 시 전체/해당일 선택
- **SDM:** 색상·가져오기 제외 필터에 `=` 정확 일치 (`and`/`or`와 조합)
- **SR:** 추출 결과 기본은 기간+제목(삭제 가능), 클릭 시 편집 필드 펼침
- **SR:** 단어 필터 설정(사전준비·시간·장소·부가정보·설명, `and`/`or`/`=`, 부가정보 append)
- **SR:** 일정 나누기 — 같은 기간으로 제목·강사 등만 다른 행 복제

### 확인
- [ ] SDM 연결 일정 삭제 확인(Win 브라우저)
- [ ] SDM `=청년회` 필터(색상·가져오기)
- [ ] SR 추출 축약/펼침 · 단어 필터 · 나누기
- [ ] Mac: 동일(브라우저 UI · 해당 없음 분기)

## 2026-09-13 — SR 추출 결과 카드 레이아웃
...
### 내용
- 추출 일정을 표 대신 카드(박스)로 표시. 상단 기간 라벨·삭제, 하단 입력 필드 나란히 정렬
- 홀·짝 교차 배경(흰색 / 연한 주황)으로 일정 구분

### 확인
- [ ] Win: 추출 후 카드·교차색·하단 필드 정렬
- [ ] Mac: 동일 (웹 UI)

### Win / Mac
- Win·Mac 동일 웹 UI. 경로·OS 분기 해당 없음.

## 2026-09-13 — SR 겹치는·병렬 일정 기간 합침 수정
...
### 내용
- `CollapseConsecutive`: 같은 날 중복(9.12~9.12 + 9.12~9.13 펼침)과 같은 날 다른 제목 병렬이 있어도 제목별로 연속·겹침 구간을 한 건으로 합침
- 강사가 둘 다 있으면 같을 때만 합침. 한쪽만 비어 있으면(일요일 비우기 등) 제목으로 이어 붙이고 있는 강사를 유지

### 확인
- [x] `NormalizerCollapse` 단위 테스트 6 통과
- [ ] Win: 추출 UI에서 연속·겹침 일정이 한 기간으로 표시
- [ ] Mac: 동일 (웹 UI)

### Win / Mac
- Win·Mac 동일 로직·웹 UI. 경로·OS 분기 해당 없음.

## 2026-09-13 — SR 연속 일정 기간 합침 · 내보내기 전 편집
...
### 내용
- 추출 후 같은 제목이 날짜로 이어지면 `yyyy.MM.dd~yyyy.MM.dd` 기간 한 건으로 표시
- 추출 결과 표에서 기간·시간·제목·강사·장소·부가정보 수정·행 추가/삭제 후 JSON 내보내기
- 내보내기 시 기간을 일별로 펼쳐 SDM `schedule-data.json` 형식 유지 (`CollapseConsecutive` / `ExpandDateRanges`)

### 확인
- [x] 단위 테스트 33 통과
- [ ] Win: UI에서 추출 → 기간 수정 → 내보내기
- [ ] Mac: 동일 (웹 UI)

### Win / Mac
- Win·Mac 동일 웹 UI·로컬 API. 경로·OS 분기 해당 없음.
